...
function getFriends(){
    ...
    var data='{"id":"'+userId+'"}';
    ajaxJson(data,getCookie("serverAddress")+"friendList.action",3);
...
function searchUser(){
    ...
    ajaxJson(data,getCookie("serverAddress")+"searchUser.action",8);
}
function requestAddFriend(){
    var email=document.getElementById("searchInput").value;
    var id=getCookie('userId');
    var data='{"email":"'+email+'","id":'+id+'}';
    ajaxJson(data,getCookie("serverAddress")+"friendRequest.action",9);
}
...
function  acceptAddFriend(elem) {
    ...
    var id=getCookie('userId');
    var data='{"id":'+id+'}';
    ajaxJson(data,getCookie("serverAddress")+"getFriendRequest.action",10);

}
function agreeAddFriend(id){
    var data='{"uid":'+getCookie('userId')+',"tid":'+id+',"approve":1}';
    ajaxJson(data,getCookie("serverAddress")+"replyAddFriend.action",11);
}
function disagreeAddFriend(id) {
    var data='{"uid":'+getCookie('userId')+',"tid":'+id+',"approve":0}';
    ajaxJson(data,getCookie("serverAddress")+"replyAddFriend.action",11);
}
